...
namespace PolyPlane.Rendering
{
    public class Cloud
    {
        ...
        public void RenderGL(GLRenderContext ctx, SKColor shadowColor, SKColor todColor, float todAngle)
        {
            // Don't even try if the viewport is above the shadows and rays.
            if (ctx.Viewport.bottom * -1f < MAX_SHADOW_ALT)
            {
                if (_altitude <= MAX_SHADOW_ALT)
                    DrawCloudGroundShadowAndRayGL(ctx, shadowColor, todAngle);
            }

            if (ctx.Viewport.Contains(this.Position, (this.Geometry.Radius * 2f) * this.Geometry.ScaleX * World.CLOUD_SCALE))
                DrawCloudGL(ctx, todColor);
        }
        ...
        private void DrawCloudGL(GLRenderContext ctx, SKColor todColor)
        {
            var color1 = _cloudColorDark.ToSKColor();
            var color2 = _cloudColorLight.ToSKColor();
            var points = this.Geometry.Points;
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                // Draw cloud part with lighting.
                var sampleLocation = point + this.Position;
                ctx.FillEllipseWithLighting(point, dims.ToSKSize(), sampleLocation, color, LIGHT_INTENSITY);
            }
        ...
        private void DrawCloudGroundShadowAndRayGL(GLRenderContext ctx, SKColor shadowColor, float todAngle)
        {
            const float WIDTH_OFFSET = 50f;
            ...
            if (ctx.Viewport.Contains(_shadowRayPoly))
            {
                ...
                if (alpha >= 0.005f)
                {
                    var rayColor = shadowColor.WithAlpha((byte)(255f * alpha));
                    ctx.FillPolygon(_shadowRayPoly, rayColor);
                }
            }
            ...
            if (ctx.Viewport.Contains(cloudShadowPos, this.Geometry.Radius * this.Geometry.ScaleX * World.CLOUD_SCALE * 3f))
            {
                // Make the ground shadow slightly wider.
                var shadowWidth = widthHalf * 1.2f;
                ctx.FillEllipse(cloudShadowPos, new SKSize(shadowWidth, 35f), shadowColor.WithAlpha(51));
            }
        }
